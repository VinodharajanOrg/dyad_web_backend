...
export function ModelsSection({ providerId }: ModelsSectionProps) {
  ...
  const { mutate: deleteModel, isPending: isDeleting } = useDeleteCustomModel({
    ...
    onSuccess: () => {
      ...
      // Note: Do NOT call refetchModels() - the mutation already invalidates the cache
    },
    onError: (error: Error) => {
    ...
    <div className="mt-8 border-t pt-6">
            ...
            >
              ...
              <div className="flex flex-wrap gap-x-2">
                <span className="mt-2 inline-block bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full dark:bg-blue-900 dark:text-blue-300">
                  {model.customProviderId ? "Custom" : "Built-in"}
                </span>
              </div>
            </div>
          ))}
      ...
      <CreateCustomModelDialog
        ...
        onSuccess={() => {
          ...
          // Note: Model cache is invalidated by the mutation in CreateCustomModelDialog
        }}
      ...
      <EditCustomModelDialog
        ...
        onSuccess={() => {
          ...
          // Note: Model cache is invalidated by the mutation in EditCustomModelDialog
        }}
        providerId={providerId}
        model={modelToEdit}
      />
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              model "
              {models?.find((m) => m.id === modelToDelete)?.displayName ||
                modelToDelete}
              ".
            </AlertDialogDescription>
          </AlertDialogHeader>
