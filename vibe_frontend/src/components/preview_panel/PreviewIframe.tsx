...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
  ...
  const { appUrl, originalUrl } = appUrlObj;

  // Debug logging for preview panel
  ...
  const { isPublishEnabled } = useDockerStatus(selectedAppId);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center p-2 border-b space-x-2 ">
        ...
        <div className="flex space-x-1">
          <button
            onClick={isPublishEnabled ? onRestart : undefined}
            disabled={!isPublishEnabled}
            className="flex items-center space-x-1 px-3 py-1 rounded-md text-sm hover:bg-[var(--background-darkest)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            title={
              isPublishEnabled
                ? "Restart App"
                : "App must be running and ready to restart"
            }
          >
            <Power size={16} />
            <span>Restart</span>
        ...
        {!appUrl ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4 bg-gray-50 dark:bg-gray-950">
            ...
            {/* <p className="text-xs text-gray-500">
              {selectedAppId ? `App ID: ${selectedAppId}` : 'No app selected'}
            </p> */}
          </div>
        ) : (
          <div
