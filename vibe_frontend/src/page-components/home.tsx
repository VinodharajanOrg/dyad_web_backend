...
import { LOADING_STATE_DELAY } from "@/lib/constants";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function HomePage() {
  ...
  // Web API mutations - enable auto-toasts to show both notifications
  ...
  const handleSubmit = async (options?: HomeSubmitOptions) => {
    ...
    try {
      ...
      const newApp = await createAppMutation.mutateAsync({
        name: appName,
        path: `/apps/${appName}`, // Default path
      });
      ...
      // Allow toasts to display before navigation
      await new Promise((resolve) => setTimeout(resolve, LOADING_STATE_DELAY));

      // NOTE: neonTemplateHook removed - Electron-only feature
      // Neon project setup would be handled by backend in web version
      // if (
      ...
      await refreshApps();
      await invalidateAppQuery(queryClient, { appId: newApp.id });
      posthog.capture("home:chat-submit");
      router.push(`/${newApp.id}/chat?id=${newChat.id}`);
    } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center justify-center max-w-3xl w-full m-auto p-8">
      ...
      <div className="w-full">
        ...
        <HomeChatInput onSubmit={handleSubmit} isStreaming={isStreaming} />
        ...
        {/* NOTE: Bypass Dyad pro banner for web */}
        {/* <ProBanner /> */}
      </div>
      <PrivacyBanner />
