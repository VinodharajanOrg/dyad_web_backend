...
export class AuthService {
  ...
  async handleCallback(code: string, redirectUri: string) {
    try {
      ...
      const ssoUser = await provider.exchangeCodeForToken(code, redirectUri);
      const { accessToken } = ssoUser;
      // Decode the ID token (do not verify here, just decode for sub)
      const decoded: any = jwt.decode(accessToken);
      const sub = decoded.oid;
      ssoUser.providerUserId = sub;
      //upsert user in db
      //take provider name from local
      const Authprovider = process.env.AUTH_PROVIDER || "azure_ad";
